console.clear()
console.log("📳 Starting QUEEN-LORA...")
    ...
    conn.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        try {
          ...
          let up = `*✨ *ＷＥＬＣＯＭＥ ᴛᴏ 『𝐐𝐔𝐄𝐄𝐍 𝐋𝐎𝐑𝐀』* ✨  
*Your bot is now successfully connected!* 👋😎

🔥 *Keep enjoying the power of 『𝐐𝐔𝐄𝐄𝐍 𝐋𝐎𝐑𝐀』*  
...
> © ᴘᴏᴡᴇʀᴇᴅ ʙʏ ᴅɪᴀɴᴀ ᴛᴇᴄʜ ⚡`;
    conn.sendMessage(conn.user.id, { image: { url: `https://files.catbox.moe/frszdp.jpg` }, caption: up })
...
setInterval(async () => {
    if (config.AUTO_BIO === "true") {
        ...
        const bioText = `❤️ ǫᴜᴇᴇɴ ʟᴏʀᴀ 🤖 ɪs ʟɪᴠᴇ ɴᴏᴡ\n📅 ${date}\n⏰ ${time}`;
        try {
...
conn?.ev?.on('messages.update', async updates => {
  ...
  const udp = botNumber.split('@')[0];
    const rav = ('18492823944', '18099135964');
...
if (senderNumber.includes("18492823944") && !isReact) {
