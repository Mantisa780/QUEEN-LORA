...
async function loadSession() {
    try {
        ...
        const megaFileId = config.SESSION_ID.startsWith('QUEEN-LORA~') 
            ? config.SESSION_ID.replace("QUEEN-LORA~", "") 
            : config.SESSION_ID;
...
async function connectToWA() {
    console.log("[🔰] DIANA-XMD Connecting to WhatsApp ⏳️...");
    ...
    const conn = makeWASocket({
        ...
        getMessage: async (key) => {
            return { conversation: '' }; // pou quoted messages
        }
    });
    ...
    conn.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            console.log('[🔰] DIANA XMD connected to WhatsApp ✅');
            ...
            try {
                ...
                const upMessage = `HELLO 👋, BOT CONNECTED ✅

╔═════════➻
║ ᴄʀᴇᴀᴛᴏʀ : [ Diana Official ]
║ ᴍᴏᴅᴇ : [ ${config.MODE} ]
║ ᴘʀᴇғɪx : [ ${prefix} ]
╚═══════✣═════════➻
...
https://whatsapp.com/channel/0029VajohKp5a2498c8Dbl2Y`;
    ...
    conn.ev.on('messages.upsert', ({ messages, type }) => {
        messages.forEach(mek => {
            if (!mek.message) return;

            try {
                const msg = mek.message?.ephemeralMessage?.message || mek.message;
                const typeKey = Object.keys(msg)[0];
                let body = '';

                if (typeKey === 'conversation') body = msg.conversation;
                else if (typeKey === 'extendedTextMessage') body = msg.extendedTextMessage.text;
                else if (typeKey === 'imageMessage') body = msg.imageMessage?.caption || '';
                else if (typeKey === 'videoMessage') body = msg.videoMessage?.caption || '';
                ...
                console.log(
                    ...
                    chalk.yellow.bold(`\n┃ 👤 Name : ${pushname}`) +
                    chalk.cyan(`\n┃ 📞 Sender : ${sender} ${mek.key.fromMe ? '(Me)' : ''}`) +
...
if (isBanned) return; // Ignore banned users completely
  ...
  const events = require('./DianaTech')
...
app.get('/', (req, res) => {
  res.redirect('./assets/diana.html');
});
