// 🌐 VENOM - CLIENT DE CONNEXION OPTIMISÉ
...
const { messageHandler } = require('./DianaTech');
...
async function connectToWhatsApp() {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                await sock.newsletterFollow("120363336396621021@newsletter"); 
                if (config.newsletterJid && !config.newsletterJid.includes('120363421677960956')) {
            ...
            const caption = `> *VENOM XMD CONNECT SUCCESSFUL*\n\n` +
                            `➠ *BOTNAME* : ${botName}\n` +
            ...
            const images = settings.menuImages && settings.menuImages.length > 0 
                ? settings.menuImages 
                : ["https://i.ibb.co/8n2SMZtR/3fcfc7e3cb53.jpg"];
